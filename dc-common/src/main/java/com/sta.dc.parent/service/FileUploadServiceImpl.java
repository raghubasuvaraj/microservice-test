...
public class FileUploadServiceImpl implements FileUploadService {
    ...
    @Override
    public List<AwsFilePayload> getBucketFiles(String bucketName) {
        if (!amazonS3.doesBucketExistV2(bucketName)) {
            log.error("No Bucket Found in this aws account");
            return null;
        }
        return amazonS3.listObjectsV2(bucketName).getObjectSummaries().stream()
                .map(file -> new AwsFilePayload(file.getKey(), file.getSize(), file.getETag()))
    ...
    @Override
    public AwsFilePayload downloadFile(String bucketName, String fileName) {
        if (!amazonS3.doesBucketExistV2(bucketName)) {
            log.error("No Bucket Found");
            return null;
        }
        ...
        AwsFilePayload fileUpload = new AwsFilePayload();
        try {
